        ...
        for devnum, device in ipairs(row.devices) do
            if not device.skip or (_P.autoskip and device.fingers <= 0) then
                local status, mosfet = xpcall(pcell.create_layout, fulltraceback, "basic/mosfet", device.name, {
                    ...
                    extendoxidetypetop = _select_switch(((rownum == #_P.rows) or _P.splitgates), _select_parameter("extendoxidetypetop", device, row), _P.separation / 2),
                    extendoxidetypebottom = _select_switch(((rownum == 1) or _P.splitgates), _select_parameter("extendoxidetypebottom", device, row), _P.separation / 2),
