...
function check(_P)
    ...
    if not (_P.ignoreseparationchecks or _P.separationautocalc) then
        ...
        if (_P.innergatestraps * _P.gatestrapwidth + (_P.innergatestraps + 1) * _P.gatestrapspace) ~= _P.separation then
            return false, string.format("the separation between nmos and pmos must have the exact size to fit %d row(s) of gate contacts (%d vs. %d)", _P.innergatestraps, _P.separation, _P.innergatestraps * _P.gatestrapwidth + (_P.innergatestraps + 1) * _P.gatestrapspace)
        end
    end
