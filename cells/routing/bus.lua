...
function layout(bus, _P)
    ...
    for merge = 1, _P.nummerge do
        ...
        for reg = 1, _P.numregs do
            local xreg
            if _P.useregorder then
                xreg = _P.regorder[reg]
            else
                xreg = reg
            end
            local yreg = reg
            local regxshift
            if _P.vregshift > 0 then
                regxshift = (xreg - 1) * _P.vregshift
            else
                regxshift = (xreg - 1) * _P.numbits * (_P.vwidth + _P.vspace)
            end
            local regyshift
            if _P.hregshift > 0 then
                regyshift = (yreg - 1) * _P.hregshift
            else
                regyshift = (yreg - 1) * _P.nummerge * _P.numbits * (_P.hwidth + _P.hspace)
            end
            for bit = 1, _P.numbits do
                local xbit = bit
                local ybit
                if _P.usebitorder then
                    ybit = _P.bitorder[bit]
                else
                    if _P.invert then
