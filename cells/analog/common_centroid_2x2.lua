...
function layout(cell, _P)
    ...
    -- center cell
    cell:move_point_x(
        cell:get_area_anchor_fmt("innerdummy1_sourcedrainactive%d", _P.innerdummies // 2 + 1).bl,
        point.create(0, 0)
    )
    ...
    if not _P.equalgatenets then
        ...
        for _, entry in ipairs({
            ...
            geometry.viabltr(cell, 1, _P.gatemetal,
                point.create(
                    ...
                    ) - _P.gateviawidth / 2 + entry.factor * gatepitch,
        ...
        cell:add_area_anchor_bltr("gateline",
            ...
            point.create(
                cell:get_area_anchor("M2r_topgatestrap").r,
                cell:get_area_anchor("M2r_active").t + separation / 2 + _P.gatelinewidth / 2
            )
        )
        geometry.rectanglebltr(cell, generics.metal(_P.gatemetal),
        ...
        geometry.viabltr(cell, 1, _P.gatemetal,
            point.create(
                0.5 * (
                    cell:get_area_anchor("M2r_topgatestrap").l +
                    cell:get_area_anchor("M2r_topgatestrap").r
                ) - _P.gateviawidth / 2,
                cell:get_area_anchor("gateline").b
            ),
        ...
        geometry.rectanglebltr(cell, generics.metal(1),
            point.create(
                ...
                cell:get_area_anchor("M2r_topgatestrap").t
            ),
            point.create(
                0.5 * (
                    cell:get_area_anchor("M1r_botgatestrap").l +
                    cell:get_area_anchor("M1r_botgatestrap").r
                ) + _P.gateconnwidth / 2,
                cell:get_area_anchor("M1r_botgatestrap").t
            )
        )
    end
