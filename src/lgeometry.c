...
static int lgeometry_viabltr(lua_State* L)
{
    ...
    if(!res)
    {
        lua_pushfstring(L, "geometry.viabltr: could not fit via. Area: (%d, %d) and (%d, %d)", bl->point->x, bl->point->y, tr->point->x, tr->point->y);
        lua_error(L);
    }
    return 0;
}
...
static int lgeometry_via(lua_State* L)
{
    ...
    if(!res)
    {
        lua_pushfstring(L, "geometry.via: could not fit via. Area: (%d, %d) and (%d, %d)", xshift - width / 2, yshift - height / 2, xshift + width / 2, yshift + height / 2);
        lua_error(L);
    }
    return 0;
}
...
static int lgeometry_unequal_ring(lua_State* L)
{
    ...
    geometry_unequal_ring(lobject_get(cell), layer, width, height, ringwidth, ringheight);
    return 0;
}
