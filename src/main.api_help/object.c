...
/* object.move_point_x */ { struct parameter parameters[] = { { "cell",       OBJECT, NULL,   "cell which should be moved" },
    { "source",     POINT,  NULL,   "source point" },
    { "target",     POINT,  NULL,   "target point" } }; vector_append(entries, _make_api_entry( "move_point_x", MODULE_OBJECT, "translate (move) the object so that the x-coorindate of the source point lies on the x-coordinate target. Usually the source point is an anchor of the object, but that is not a necessity. The points are just references for the delta vector and can be any points.",
            "cell:move_point_x(cell:get_area_anchor(\"gate\").bl, point.create(0, 0)) -- move the x-coordinate of the origin\nmosfet:move_point_x(mosfet:get_area_anchor(\"leftsourcedrain\").bl, othermosfet:get_area_anchor(\"rightsourcedrain\").bl) -- align the x-coordinate of two mosfets", parameters, sizeof(parameters) / sizeof(parameters[0]))); }
...
/* object.move_point_y */
{
    ...
    vector_append(entries, _make_api_entry(
        ...
        "cell:move_point_y(cell:get_area_anchor(\"gate\").bl, point.create(0, 0)) -- move the y-coordinate of the origin\nmosfet:move_point_y(mosfet:get_area_anchor(\"leftsourcedrain\").bl, othermosfet:get_area_anchor(\"rightsourcedrain\").bl) -- align the y-coordinate of two mosfets", parameters, sizeof(parameters) / sizeof(parameters[0]))); } /* object.add_child */ { struct parameter parameters[] = { { "cell",      OBJECT, NULL, "Object to which the child is added" },
        { "child",     OBJECT, NULL, "Child to add" },
        { "instname",  STRING, NULL,   "Instance name (not used by all exports)" }, }; vector_append(entries, _make_api_entry( "add_child", MODULE_OBJECT, "Add a child object (instance) to the given cell. This make 'cell' the parent of the child (it manages its memory). This means that you should not use the original child object any more after this call (unless it is object.add_child or object.add_child_array)",
