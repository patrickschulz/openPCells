...
/* placement.optimize */
{
    ...
    vector_append(entries, _make_api_entry(
        "optimize",
        MODULE_PLACEMENT,
        "run a placement optimization",
        "Minimize wire length by optimizing the placement of the instances by a simulated annealing algorithm. "
        "This function returns a table with the rows and columns of the placement of the instances. "
        "It is intended to be called in a place-and-route-script for --import-verilog",
...
/* placement.manual */
{
    ...
    vector_append(entries, _make_api_entry(
        "manual",
        MODULE_PLACEMENT,
        "execute a manual placement specification",
        "Create a placement of instances manually. "
        "This function expects a row-column table with all instance names. "
        "Thus the instance names must match the ones found in the instances table (from the verilog netlist). "
        "This function then updates all required references in the row-column table, that are needed for further processing (e.g. routing). "
        "This function is useful for small designs, especially in a hierarchical flow",
...
/* placement.insert_filler_names */
{
    ...
    vector_append(entries, _make_api_entry(
        "insert_filler_names",
        MODULE_PLACEMENT,
        "insert fillers in standard cell placements",
...
/* placement.rowwise */
{
    ...
    vector_append(entries, _make_api_entry(
        "rowwise",
        MODULE_PLACEMENT,
        "place cells in a row-wise manner in a parent cell",
        "Place cells in a row-wise manner in a parent cell. "
        "The cells definition contains definitions for every row, which in turn contain entries with two keys: 'reference' (an object) and 'instance' (an instance name). "
        "The placed cells are aligned by their alignment boxes and grow into the upper-right direction. "
...
/* placement.columnwise */
{
    ...
    vector_append(entries, _make_api_entry(
        "columnwise",
        MODULE_PLACEMENT,
        "place cells in a column-wise manner in a parent cell",
        "Place cells in a column-wise manner in a parent cell. "
        "The cells definition contains definitions for every column, which in turn contain entries with two keys: 'reference' (an object) and 'instance' (an instance name). "
        "The placed cells are aligned by their alignment boxes and grow into the upper-right direction. "
...
/* placement.place_at_origins */
{
    ...
    vector_append(entries, _make_api_entry(
        "place_at_origins",
        MODULE_PLACEMENT,
        "place cells in a toplevel cells at the specified origins",
        "Place cells in a toplevel cells at the specified origins. "
        "The instances are named accordingly to the basename (with _1, _2, etc. appended). "
        "This is a more low-level placement function (compared to placement.place_within_boundary), which is called by the higher-level functions. "
        "In some cases, using this function directly can be useful. "
...
/* placement.place_on_grid */
{
    ...
    vector_append(entries, _make_api_entry(
        "place_on_grid",
        MODULE_PLACEMENT,
        "place cells in a toplevel cells corresponding to the given grid",
        "Place cells in a toplevel cells corresponding to the given grid. "
        "The instances are named accordingly to the basename (with _1, _2, etc. appended). "
...
/* placement.place_within_boundary */
{
    ...
    vector_append(entries, _make_api_entry(
        ...
        MODULE_PLACEMENT,
        "automatically place a cell multiple times in a toplevel cell (hierarchical)",
        "Automatically place a cell multiple times in a toplevel cell. "
        "The cell instances will be placed in the given target area and given names based on the given basename. "
...
/* placement.place_within_rectangular_boundary */
{
    ...
    vector_append(entries, _make_api_entry(
        ...
        MODULE_PLACEMENT,
        "place fill in a rectangular boundary",
        "Place fill in a rectangular boundary. "
        "This function behaves like placement.place_within_boundary, but it takes the corner points (bottom-left and top-right) as inputs. "
...
/* placement.place_within_layer_boundaries */
{
    ...
    vector_append(entries, _make_api_entry(
        ...
        MODULE_PLACEMENT,
        "place cells in a boundary based on their layer content",
        "Place cells in a boundary based on their layer content. "
        "This function is similar to placement.place_within_boundary, but uses non-binary excludes. "
        "A look-up table with cells is given, that defines the occupied layers of these cells and places only cells that don't have content in the excluded layers. "
        "The layerexcludes table contains the excludes in the respective layers. "
        "This function tries to maximize the number of placed cells, starting for every point with the first cell. "
        "After a cell is placed, its layers are used to block that region. "
        "That means that if cells exist with non-overlapping layer content, it is possible that multiple cells are placed per grid point. "
...
/* placement.place_boundary_grid */
{
    ...
    vector_append(entries, _make_api_entry(
        "place_boundary_grid",
        MODULE_PLACEMENT,
        "place cells on a regular grid with the given pitch",
        "Place cells on a regular grid with the given pitch. "
        "The grid contains numeric entries of either 1 or 0, meaning 'place' or 'don't place'. "
        "This grid can be obtained by using placement.calculate_grid. "
