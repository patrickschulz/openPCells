...
/* For lengths, 1 unit is equal to 0.01 um. */
const int floorplan_width = 3686, floorplan_height = 3640;
const int site_height = 140, site_width = 19;
...
static inline bool net_valid(struct net* n)
{
    return n->net_name;
}

static inline bool cell_valid(struct cell* c)
{
...
void update_net_struct_ptrs(struct net* all_nets, struct cell* all_cells)
{
    struct cell* c;
    struct net* n;
    struct net** n_p;
    ...
    for(n = all_nets; net_valid(n); n++)
    {
        pin_idx = 0;
        n->halfperi_wirelength = 0;
        for(c = all_cells; cell_valid(c); c++)
        {
            for(n_p = c->net_conn; *n_p; n_p++)
            {
                if(*n_p == n)
...
int get_total_wirelength(bool initial, struct net* all_nets)
{
    if(initial)
    {
        total_wirelength = 0;
...
void update_cell_count(struct cell* all_cells)
{
    struct cell* c;
    cell_count = 0;
    cell_total_width = 0;
...
void get_cells_of_row(struct cell* all_cells, struct cell** cells_in_row, int cur_row)
{
    struct cell* c;
    int cur_cell_idx = 0;
    for(c = all_cells; cell_valid(c); c++)
...
double get_legality_penalty(struct cell* all_cells)
{
    ...
    for(cur_row = 0; cur_row < floorplan_height/site_height - 1; cur_row++)
    {
        ...
        get_cells_of_row(all_cells, cells_in_row, cur_row);
...
void write_cell_locations(struct cell* all_cells, char* fn)
{
    struct cell* c;
    FILE* f = fopen(fn, "w");
    ...
    for(c = all_cells; cell_valid(c); c++)
    {
        fprintf(f, "\t{{%s} %i.%02i %i.%02i}\n", c->instance_name, c->pos_x/100, c->pos_x*100, c->pos_y/100, c->pos_y*100);
    }
    fprintf(f, "}\n");
...
void place_initial_random(struct cell* all_cells) {
    struct cell* c;
...
double get_total_penalty(struct net* all_nets, struct cell* all_cells)
{
    int wirelength = get_total_wirelength(false, all_nets);
    double legality_penalty = get_legality_penalty(all_cells);
    double total_penalty = weight_wirelength * wirelength + legality_penalty;
    return total_penalty;
}

void report_status(struct net* all_nets, struct cell* all_cells)
{
    int wirelength = get_total_wirelength(false, all_nets);
    double legality_penalty = get_legality_penalty(all_cells);
    double total_penalty = weight_wirelength * wirelength + legality_penalty;
...
int lplacer_place(lua_State* L)
{
    ...
    lua_len(L, 2);
    size_t numcells = lua_tointeger(L, -1);
    lua_pop(L, 1);

    // initialize all_nets
    struct net* all_nets = calloc(numnets + 1, sizeof(struct net));
    for(size_t i = 1; i <= numnets; ++i)
    {
        lua_geti(L, 1, i);
        size_t len = 0;
    ...
    struct cell* all_cells = calloc(numcells + 1, sizeof(struct cell));
    ...
    get_total_wirelength(true, all_nets);
    ...
    while(temperature > 0.01)
    {
        for(move_ctr = 0; move_ctr < moves_per_cell_per_temp * cell_count; move_ctr++)
        {
            ...
            double total_penalty = get_total_penalty(all_nets, all_cells);
