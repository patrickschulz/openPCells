...
local function _ask_layer_base(name, prompt, tech, options)
    ...
    if options.askname then
        entry.name = input.question("What is this layer called")
    end
    if options.askGDS then
        entry.gds = {
            layer = input.number("What is it's GDSII layer number"),
            purpose = input.number("What is it's GDSII purpose number"),
        }
    end
    if options.askSKILL then
        entry.SKILL = {
            layer = input.question("What is it's SKILL layer name"),
            purpose = input.question("What is it's SKILL purpose name"),
        }
    end
...
function M.techfile()
    ...
    local libname = input.question("What is the name of the library")
    ...
    local options = {}
    options.askGDS = input.yesno("Do you want to specify GDS layer information for the layers")
    options.askSKILL = input.yesno("Do you want to specify SKILL layer information for the layers")
    options.askname = input.yesno("For debugging purposes, it can be useful to assign a name for every layer. Do you want to be asked for layer names")
    ...
    tech.nummetals = input.number("How many metals does this metal stack have (ALL metals, including any possible top-level aluminum interconnect layer")
