...
function M.create_proxy(reference, identifier)
    local self = {
        reference = reference,
...
function meta.copy(self)
    ...
    for i, child in ipairs(self.children) do
        new.children[i] = { reference = child.reference, x0 = child.x0, y0 = child.y0, orientation = child.orientation }
    end
    return new
end
...
function meta.add_child_link(self, identifier)
    ...
    if not reference then
        moderror("trying to access an unknown child reference. Make sure to use the name generated by add_child_reference()")
    end
    local child = M.create_proxy(reference, identifier)
...
function meta.flatten(self)
    ...
    for _, child in self:iterate_children_links() do
        local obj = child.reference:copy() -- FIXME: is copy necessary?
        obj:flatten()
        -- apply transformation
        if child.orientation == "fy" then
            obj:flipy(-child.y0 / 2)
            --obj:flipy()
        end
        self:merge_into(obj, child.x0, child.y0)
    end
    -- remove children
...
function meta.inherit_alignment_box(self, other)
    ...
    if self.alignmentbox then
        ...
        self.alignmentbox = { 
            bl = point.create(math.min(blx, sblx + self.x0), math.min(bly, sbly + self.y0)), 
            tr = point.create(math.max(trx, strx + self.x0), math.max(try, stry + self.y0))
        }
    else
        self.alignmentbox = { bl = bl:copy(), tr = tr:copy() }
    end
...
function meta.get_anchor(self, name)
    if self.isproxy then
        ...
        local dx, dy = 0, 0
        if self.orientation ~= "R0" then
            local _x, _y = self.reference:get_transformation_correction()
            if self.orientation == "fx" then
                dx = _x
            elseif self.orientation == "fy" then
                dy = _y
            elseif self.orientation == "R180" then
        ...
        if aux.find(_reserved_anchors, function(n) return n == name end) then
            anchor:translate(self.x0, self.y0)
        else
