...
function meta.add_child_link(self, identifier, where)
    local other = self.children.lookup[identifier]
    if not other then
    ...
    table.insert(self.children, { origin = point.create(x + other.x0, y + other.y0), identifier = identifier })
end

function meta.add_child(self, other, name, where)
    local identifier = self:add_child_reference(other, name)
    self:add_child_link(identifier, where)
end
