...
local m3excludes = cell:get_shape_outlines(generics.metal(3), pspace)
local m4excludes = cell:get_shape_outlines(generics.metal(4), pspace)

local boundary = cell:get_boundary()
local pbl, ptr = util.polygon_rectangular_boundary(boundary)
local vlines = layouthelpers.place_vlines(cell,
    pbl, ptr,
    generics.metal(3),
    pwidth, pspace, -- width/space
    pwidth, -- minimum width
    { "vdd", "vss" },
    m3excludes
)
local hlines = layouthelpers.place_hlines(cell,
    pbl, ptr,
    generics.metal(4),
    pwidth, pspace, -- width/space
    pwidth, -- minimum width
