...
local mallexcludes = cell:get_shape_outlines(util.foreach(util.range(1, 3), generics.metal), 200)

--[[ Annotate real excludes:
for _, exclude in ipairs(mallexcludes) do
    if util.is_rectilinear_polygon(exclude) then
        local rects = util.split_rectilinear_polygon(exclude)
        for _, r in ipairs(rects) do
            geometry.rectanglepoints(cell, generics.special(), r.pt1, r.pt2)
        end
    else
