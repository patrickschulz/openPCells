...
\end{shellcode}
Go to the repository, in the main directory is a file called \shellinline{setup.sh}, which creates the main program. Execute it:
\begin{shellcode}
    > ./setup.sh
    * generating 'opc' *
...
\begin{shellcode}
    no cell type given
\end{shellcode}
This indicates that everything is working fine. Due to \shellinline{./setup.sh}, the paths for the toolchain are already set. You should be able to call
...
\end{shellcode}
The gds interface creates a \shellinline{openPCells.gds} file, the virtuoso interface a \shellinline{openPCells.il}, which can be read with a text editor. Further
