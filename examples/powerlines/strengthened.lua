...
cell:add_net_shape("vdd", point.create(4000, 1000), point.create(6000, 2000), generics.metal(1))
...
geometry.rectanglebltr(cell, generics.metal(1), point.create(4000, 9000), point.create(6000, 10000))
geometry.rectanglebltr(cell, generics.metal(1), point.create(4500, 9000), point.create(6500, 10000))
geometry.rectanglebltr(cell, generics.metal(1), point.create(5000, 9000), point.create(7000, 10000))
...
local m3excludes = cell:get_shape_outlines(generics.metal(3), 200)
...
local netshapes = {}
util.insert_table(netshapes, cell:get_net_shapes("vdd"))
...
-- add lower-level metals to strengthen grid
local m2excludes = cell:get_shape_outlines(generics.metal(2), 200)
local m2hnets = layouthelpers.place_hlines(
    cell,
    target.bl, target.tr,
    generics.metal(2),
    500, 500,
